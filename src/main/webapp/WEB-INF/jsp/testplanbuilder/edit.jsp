    ...
    <c:forEach items="${editForm.testSteps}" var="child" varStatus="loop">
      <c:if test="${child!=''}">
        <div class="row" id="inputParamRow${loop.index}">
          ...
          <div class="col-sm-3 cheil-select" style="display:none" id="fieldInputDiv${loop.index}">
            <select readonly="true" name="testSteps[${loop.index}].enumName" class="3col active" id="fieldInput${loop.index}"></select>
          </div>
          <div class="col-sm-1">
        ...
        <script type="text/javascript">
          $(document).ready(function() {
            ...
            onClassSelected('${editForm.testSteps[loop.index].className}', ${loop.index}, '${editForm.testSteps[loop.index].methodName}', '${editForm.testSteps[loop.index].enumName}');
          });
        </script>
      </c:if>
    </c:forEach>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $(".add-input-more").click(function(){
      ...
      var html = '<div class="row" id="inputParamRow' + count + '">'
              ...
              + '</div>'
              + '<div class="col-sm-3 cheil-select" style="display:none" id="fieldInputDiv' + count + '">'
              + '<select name="testSteps[' + count + '].enumName" id="fieldInput' + count + '" class="3col active"></select>'
              + '</div>'
              + '<div class="col-sm-1">'
  ...
  //presetEnumValue - the value that the Enum dropdown should be set to.
  function onClassSelected(value, countVal, presetMethodValue, presetEnumValue) {
    if(value!=''){
      ...
      $.ajax({
        ...
        success: function(data){
          ...
          if (presetMethodValue !== undefined) {
            ...
            onMethodSelected(presetMethodValue, countVal, presetEnumValue)
          }
        },
        error:function(e){
  ...
  //presetEnumValue - the value that the Enum dropdown should be set to
  function onMethodSelected(value, countVal, presetEnumValue) {
      ...
      if(value=='retrieveFieldFromPage'){ //only appears if selected method is "retrieveFieldFromPage"
        ...
        $.ajax({
          ...
          success: function(data){
            ...
            // Check if a presetEnumValue is provided and set it as the selected value
            if (presetEnumValue !== undefined) {
              $('#fieldInput' + countVal).val(presetEnumValue);
            }
          },
          error:function(e){
            console.log(e.statusText);
