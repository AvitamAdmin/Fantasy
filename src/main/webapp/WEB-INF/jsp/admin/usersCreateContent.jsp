         ...
         <div class="row">
             <div class="col-sm-12">
                 <c:forEach items="${roles}" var="role">
                     <spring:eval expression="@environment.getProperty('${role.name}')" var="roleName" />
                     <c:choose>
                         <c:when test="${fn:contains( userForm.roles, role ) }">
                           <span> &nbsp;&nbsp;<strong>${roleName}</strong> <input type="checkbox" id="${role.name}" name="roles" value="${role.id}" checked="checked">&nbsp;&nbsp;</span><br/><br/>
                         </c:when>
                         <c:otherwise>
                            <span>&nbsp;&nbsp;<strong>${roleName}</strong> <input type="checkbox" id="${role.name}" name="roles" value="${role.id}">&nbsp;&nbsp;</span><br/><br/>
                         </c:otherwise>
                     </c:choose>
                 </c:forEach>
              </div>
