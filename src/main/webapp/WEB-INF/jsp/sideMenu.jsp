                ...
                <div class="collapse" id="submenu1${loop.index}" aria-expanded="false">
                    <ul class="flex-column pl-2 nav secondmenu">
                        <c:forEach var="childNode" items="${node.childNodes}" varStatus="loop2">
                            <li class="nav-item">
                                <a class="nav-link text-truncate collapsed py-1" id="secondmenu${loop2.index}" href="#" onclick="javascript:fire_ajax_submit('${childNode.path}')"><i class="ti-menu-alt menu-icon"></i><span>${childNode.name}</span></a>
                            </li>
                        </c:forEach>
                        </ul>
                    </div>
